...
/**
 * Convert a YAML source file to a CSV file.
 * @param {String} sourcePath The YAML source file path.
 * @param {String} variableName The name of the variable in the YAML source.
 * @param {String} csvName The name of the output CSV file.
 */
function yamlToCSV(sourcePath, variableName, csvName) {
  let doc;
  ...
  doc[variableName].forEach((entry, index) => {
    ...
    csvContents += '"' + hexValue + '","' + safeName + '"';
    csvContents += isLastEntry ? '' : '\r\n';
  });

  // Write the CSV file
  fs.writeFile(csvPath, csvContents, 'utf8', (err) => {
    if(err) {
      console.log('Error writing ' + csvName + ': ' + err);
    }
    else {
...
module.exports.yamlToCSV = yamlToCSV;